<?php
    ...
    $query1 = "SELECT * FROM `inventory` WHERE `Quantity` > 0";
    ...
    if(isset($_POST['submit']))
    {
        ...
        if($table == TRUE && $sisa >= $qty)
        {
            $newqty = $sisa - $qty;
            $query = "UPDATE inventory SET `Quantity` = '$newqty' WHERE `Kode` = '$kode'";
            $result = mysqli_query($conn1, $query);
            $query = "INSERT INTO `history` (`Date`, `Kode`, `Quantity`, `Status`) values (CURRENT_DATE, '$kode', '$qty', 'Keluar')";
            $result = mysqli_query($conn1, $query);
            header("location:out.php?success=1");
        }
        else
        {
            header("location:out.php?error=1");
        }
    }
?>
...
<html lang="en">
  ...
  <body>
    ...
    <div class="container">
        <h1 class="page-header">Keluar Barang
        </h1>
        ...
        <div class="form-group">
          ...
          <?php
            ...
            {
                echo '<font color="#FF0000"><p align="center">Record failed! Barang tidak cukup / Salah Kode';
            }
